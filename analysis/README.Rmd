...
phase_by_elev_plot
save_plot(filename = "figures/uncert_fig03_phase_by_elev.png",
          plot = phase_by_elev_plot, 
...
cumulative_rs_plot <-
  obsPass %>% 
  mutate(wy2 = paste0("Water Year ", wy)) %>% 
  ggplot(aes(dowy, n_cumulative, color = phase)) + 
  geom_line(lwd = 1) + 
  labs(x = "Date",
       y = "Cumulative Observations") +
  theme(legend.position = c(0.08, 0.8), legend.title = element_blank(),
        strip.background = element_rect(fill = NA, color = "black")) +
  phase_color_scale2 +
  facet_wrap(~wy2, ncol = 1) +
  scale_x_continuous(breaks = c(1, 93, 183, 274), 
                     labels = c("Oct. 1", "Jan. 1", "Apr. 1", "Jul. 1"))
cumulative_rs_plot
save_plot(filename = "figures/uncert_fig02_cumulative_rs.png",
          plot = cumulative_rs_plot, 
          base_width = 6, base_height = 3.5)
```
...
```{r echo = FALSE, warning = FALSE, message = FALSE}
temperature_phase_density_plot <-
  plot_grid(
...
temperature_phase_density_plot
save_plot(filename = "figures/uncert_fig04_temperature_phase_density.png",
          plot = temperature_phase_density_plot, 
...
cond_prob_plot
save_plot(filename = "figures/uncert_fig05_conditional_probability.png",
          plot = cond_prob_plot, 
          base_width = 6, base_height = 4)
...
rs_line_hist_plot
save_plot(filename = "figures/uncert_fig06_rs_line_hist.png",
          plot = rs_line_hist_plot, 
...
phase_freq_plot <-
  ...
  theme(legend.position = "none")
phase_freq_plot
save_plot(filename = "figures/uncert_fig07_phase_freq.png",
          plot = phase_freq_plot, 
          base_width = 6, base_height = 4)
```
...
method_success_plot
save_plot(filename = "figures/uncert_fig08_method_success.png",
...
gpm_success_plot
save_plot(filename = "figures/uncert_fig09_gpm_success.png",
          plot = gpm_success_plot, 
          base_width = 6, base_height = 4)
```
...
gpm_phase_plp_plot
save_plot(filename = "figures/uncert_fig12_gpm_phase_plp.png",
          plot = gpm_phase_plp_plot, 
...
rs_line_fit_event_plot
save_plot(filename = "figures/uncert_fig10_rs_line_fit_event.png",
          plot = rs_line_fit_event_plot, 
...
We found a mean bias of `r tair_mean_bias_all`°C and an r<sup>2</sup> of `r tair_r2_all` when we compared the imputed air temperature values to the originally recorded observations using all available data. When examining only air temperature values between -5°C and 10°C on days with crowdsourced observations, we found a mean bias of `r tair_mean_bias_neg05_10_ppt`°C and an r<sup>2</sup> of `r tair_r2_neg05_10_ppt`. The error metrics for modeled wet bulb and dew point temperature and relative humidity were all similarly encouraging. However, we do note that small errors in the meteorological data can propogate into uncertainty in the amounts of rain versus snow predicted by the different precipitation phase partitioning methods. 

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.dim= c(5,5)}
tair_dis_val_plot <- 
  ggplot(tair_model_validation, aes(tair, tair_idw_lapse_var)) +
  geom_point(alpha = 0.2) +
  geom_abline(slope = 1, intercept = 0, lwd = 1, lty = "dashed", color = "darkred") +
  annotate(geom = "text", x = -10, y = -18, 
             label = "1:1 Line", color = "darkred", hjust = 1) + 
  geom_smooth(method = "lm", se = F, color = "darkblue") + 
  annotate(geom = "text", x = 20, y = 32, 
             label = "Fit Line", color = "darkblue", hjust = 0) + 
  labs(x = "Observed Air Temperature (°C)",
       y = "Modeled Air Temperature (°C)")
tair_dis_val_plot

tdew_dis_val_plot <- 
  ggplot(tdew_model_validation, aes(tdew, tdew_idw_lapse_var)) +
  geom_point(alpha = 0.2) +
  geom_abline(slope = 1, intercept = 0, lwd = 1, lty = "dashed", color = "darkred") +
  annotate(geom = "text", x = -20, y = -24, 
             label = "1:1 Line", color = "darkred", hjust = 0) + 
  geom_smooth(method = "lm", se = F, color = "darkblue") + 
  annotate(geom = "text", x = -21, y = -18, 
             label = "Fit Line", color = "darkblue", hjust = 1) + 
  labs(x = "Observed Dew Point Temperature (°C)",
       y = "Modeled Dew Point Temperature (°C)")
tdew_dis_val_plot

tdew_eq_val_plot <- 
  ggplot(tdew_equation_validation, aes(tdew, tdew_est)) +
  geom_point(alpha = 0.2) +
  geom_abline(slope = 1, intercept = 0, lwd = 1, lty = "dashed", color = "darkred") +
  annotate(geom = "text", x = -20, y = -24, 
             label = "1:1 Line", color = "darkred", hjust = 0) + 
...
twet_eq_val_plot <- 
  ggplot(twet_equation_validation, aes(twet, twet_est)) +
  geom_point(alpha = 0.2) +
  geom_abline(slope = 1, intercept = 0, lwd = 1, lty = "dashed", color = "darkred") +
  annotate(geom = "text", x = -15, y = -18, 
             label = "1:1 Line", color = "darkred", hjust = 0) + 
  geom_smooth(method = "lm", se = F, color = "darkblue") + 
  annotate(geom = "text", x = -15, y = -11, 
             label = "Fit Line", color = "darkblue", hjust = 1) + 
  labs(x = "Observed Wet Bulb Temperature (°C)",
       y = "Modeled Wet Bulb Temperature (°C)")
twet_eq_val_plot

# Save the figs
save_plot(filename = "figures/uncert_figs01_tair_dis_val.png",
          plot = tair_dis_val_plot,
          base_width = 5, base_height = 5)
save_plot(filename = "figures/uncert_figs02_tdew_dis_val.png",
          plot = tdew_dis_val_plot,
          base_width = 5, base_height = 5)
save_plot(filename = "figures/uncert_figs03_tdew_eq_val.png",
          plot = tdew_eq_val_plot,
          base_width = 5, base_height = 5)
save_plot(filename = "figures/uncert_figs04_twet_eq_val.png",
          plot = twet_eq_val_plot,
          base_width = 5, base_height = 5)
...
obs_time_plot
save_plot(filename = "figures/uncert_fig11_obs_time.png",
          plot = obs_time_plot, 
          base_width = 6, base_height = 4)
```
...
```{r echo = FALSE, warning = FALSE, message = FALSE}
# Import the rain-snow probability by elevation data
rs_line_by_elev <- readRDS("../data/processed/mros_obs_rain_snow_prob_elev_2020-2021.RDS")

rs_line_example_plot <- 
  ggplot(filter(rs_line_by_elev, date == as.Date("2021-01-28")),
         aes(elev, snow_prob)) + 
    geom_point() +
    geom_smooth(method = "lm", se = F) + 
    geom_hline(yintercept = 50, lty = "dashed") +
    labs(x = "Elevation (m)", y = "Snowfall Probability (%)") +
    annotate(geom = "text", x = 1100, y = 15, 
             label = "Fit Line", color = "darkblue", hjust = 1)
rs_line_example_plot
save_plot(filename = "figures/uncert_figs05_rs_line_example.png",
          plot = rs_line_example_plot,
          base_width = 5, base_height = 5)
```
...
```
Of the `r length(valid_dates)` days with a valid rain-snow line estimate from the citizen science data, there were `r rs_line_count` days with corresponding measurements of the brightband elevation from the freezing-level radar. There was a reasonable relationship between the two datasets with an r<sup>2</sup> of `r rs_line_r2_trim` (Fig XXXXa). When comparing the daily data, there was a slight negative bias in the radar-derived values of `r rs_line_bias_trim` m. This pattern can be seen in the example shown in Fig. XXXXb, where the hourly freezing-level radar measurements are typically below the daily estimates from the crowdsourced data. In general, the fluctuation in the brightband elevation was relatively small on days of overlapping data, with an average range of `r round(rs_line_hrly_range_av_trim, 1)` m. The day with the greatest difference between minimum and maximum elevations was `r flr_rs_summary_trim %>% slice(which(rs_line_range == rs_line_hrly_range_max_trim)) %>% pull(date)` at `r rs_line_hrly_range_max_trim` m  and the day with the minimum difference was `r flr_rs_summary_trim %>% slice(which(rs_line_range == rs_line_hrly_range_min_trim)) %>% pull(date)` at `r rs_line_hrly_range_min_trim` m.
...
rs_line_fit_event_plot_trim <- 
  plot_grid(
  ggplot(rs_line_daily_trim, aes(rs_line.x, rs_line.y)) + 
    geom_point() + 
    geom_smooth(method = "lm", se = F, color = "darkblue") + 
    geom_abline(slope = 1, intercept = 0, lwd = 1, lty = "dashed", color = "gray") +
    annotate(geom = "text", x = 2000, y = 1650, 
...
rs_line_fit_event_plot_trim
save_plot(filename = "figures/uncert_fig10_rs_line_fit_event_trim.png",
          plot = rs_line_fit_event_plot_trim, 
          base_width = 8, base_height = 4)
```
...
phase_by_elev_plot_hyp <- 
  plot_grid(
    ggplot(obsPass, aes(elev, fill = phase)) + 
        ...
        theme(strip.background = element_rect(fill = "white", color = "black"),
              legend.position = c(0.8, 0.8)) +
        xlim(min(domain_elev$elev_m), max(obsPass$elev)),
    ggplot(domain_elev, aes(elev_m)) + 
        stat_ecdf(geom = "step") +
        coord_flip()+
        xlim(min(domain_elev$elev_m), max(obsPass$elev)) +
        ...
        labs(y = "Domain Fraction"),
    rel_widths = c(0.7, 0.3), labels = "auto"
)
phase_by_elev_plot_hyp
save_plot(filename = "figures/uncert_fig03_phase_by_elev_hyp.png",
          plot = phase_by_elev_plot_hyp, 
          base_width = 7, base_height = 4.5)
```
