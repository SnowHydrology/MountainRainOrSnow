...
validate <- function(temp_min, temp_max, valid_days, temp_df, n_samples){
  ...
  rows_to_remove = sample(1:length(temp_tmp$tair), n_samples, replace=F)
  tair_test = temp_tmp[rows_to_remove, ]
  tair_validate = temp_tmp[-rows_to_remove, ]
...
# Add date column to tair data
tair$date <- as.Date(tair$datetime, tz = "Etc/GMT+8")
...
tmin = c(min(tair$tair, na.rm = T), -10, -5, -10, -5)
tmax = c(max(tair$tair, na.rm = T), 20, 10, 20, 10)
...
for(j in 1:length(scenarios)){
  validate_tmp <- validate(temp_min = tmin[j], 
                           ...
                           temp_df = tair,
                           n_samples = nSamples)
  validate_tmp <- Filter(function(x) length(x) == 16, validate_tmp)
