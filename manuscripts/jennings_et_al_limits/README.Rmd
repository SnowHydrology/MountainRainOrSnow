...
The two machine learning methods exhibited similar accuracy patterns by air temperature as the best and median benchmark methods, with performance degrading at air temperatures slightly above freezing. Both random forest and XGBoost achieved their minimum success rates of `r filter(summary_bytemp, source == "cs" & scenario == "nomix_imbal" & ppm == "rf") %>% pull(accuracy_pct) %>% min() %>% round(digits = 1)`% and `r filter(summary_bytemp, source == "cs" & scenario == "nomix_imbal" & ppm == "xg") %>% pull(accuracy_pct) %>% min() %>% round(digits = 1)`%, respectively, at 2°C in the crowdsourced dataset. Performance was higher in the synoptic dataset with minimum accuracy values of `r filter(summary_bytemp, source == "nh" & scenario == "nomix_imbal" & ppm == "rf") %>% pull(accuracy_pct) %>% min() %>% round(digits = 1)`% for random forest and `r filter(summary_bytemp, source == "nh" & scenario == "nomix_imbal" & ppm == "xg") %>% pull(accuracy_pct) %>% min() %>% round(digits = 1)`% for XGBoost. Both minima occurred at 1°C in the synoptic dataset. Relative improvements in accuracy compared to the best and median benchmarks varied by temperature for the machine learning methods (Figure 4). In the crowdsourced dataset, relative improvements reached a maximum of `r filter(ml_benchmark_diff_bytemp, source == "cs" & benchmark == "best") %>% pull(accuracy_diff_rel_pct) %>% max() %>% round(digits = 1)`% at `r filter(ml_benchmark_diff_bytemp, source == "cs" & benchmark == "best") %>% slice_max(accuracy_diff_rel_pct) %>% pull(tair_bin) %>% round(digits = 1) `°C compared to the best benchmark and a maximum of `r filter(ml_benchmark_diff_bytemp, source == "cs" & benchmark == "med") %>% pull(accuracy_diff_rel_pct) %>% max() %>% round(digits = 1)`% at `r filter(ml_benchmark_diff_bytemp, source == "cs" & benchmark == "med") %>% slice_max(accuracy_diff_rel_pct) %>% pull(tair_bin) %>% round(digits = 1) `°C compared to the median benchmark. In the synoptic dataset relative improvements were smaller for the best benchmark comparison, with a max increase of `r filter(ml_benchmark_diff_bytemp, source == "nh" & benchmark == "best") %>% pull(accuracy_diff_rel_pct) %>% max() %>% round(digits = 1)`% at `r filter(ml_benchmark_diff_bytemp, source == "nh" & benchmark == "best") %>% slice_max(accuracy_diff_rel_pct) %>% pull(tair_bin) %>% round(digits = 1) `°C, and larger for the median benchmark, with a max increase of `r filter(ml_benchmark_diff_bytemp, source == "nh" & benchmark == "med") %>% pull(accuracy_diff_rel_pct) %>% max() %>% round(digits = 1)`% at `r filter(ml_benchmark_diff_bytemp, source == "nh" & benchmark == "med") %>% slice_max(accuracy_diff_rel_pct) %>% pull(tair_bin) %>% round(digits = 1) `°C.

When compared to the snow and rain biases produced by the best and median benchmark methods, the machine learning methods provided larger absolute improvements than those recorded for accuracy (Table YYYY). While accuracy improvements were all less than or equal to `r max(ml_benchmark_diff$accuracy_pct_diff) %>% round(digits = 1)`%, the reduction in snow bias maxed out at `r filter(ml_benchmark_diff, source == "cs") %>% pull(snow_bias_abs_pct_diff) %>% min() %>% round(digits = 1)`% and `r filter(ml_benchmark_diff, source == "nh") %>% pull(snow_bias_abs_pct_diff) %>% min() %>% round(digits = 1)`% in the crowdsourced and synoptic datasets, respectively. Random forest and XGBoost provided an even larger improvement in rain bias in the crowdsourced dataset with a maximum reduction of `r filter(ml_benchmark_diff, source == "cs") %>% pull(rain_bias_abs_pct_diff) %>% min() %>% round(digits = 1)`%. Conversely, the rain bias reduction was slightly smaller than the snow bias reduction in the synoptic dataset at `r filter(ml_benchmark_diff, source == "nh") %>% pull(rain_bias_abs_pct_diff) %>% min() %>% round(digits = 1)`%. These results show that while accuracy improvements from the machine learning methods may be small, random forest and XGBoost predict rain and snow with lower biases than the benchmark methods.

## Mixed precipitation

Until this point, we have only considered precipitation in its solid and liquid forms. While the synoptic dataset does not include any mixed precipitation observations, the testing split of the crowdsourced dataset includes `r filter(met, source == "cs" & scenario == "allphase_imbal" & phase == "mix") %>% nrow()` observations, comprising `r ((filter(met, source == "cs" & scenario == "allphase_imbal" & phase == "mix") %>% nrow()) / (filter(met, source == "cs" & scenario == "allphase_imbal") %>% nrow()) * 100) %>% round(digits = 1)`% of the total. As noted in the methods section, we do not include any dual-threshold benchmark methods because of their poor historical performance. The machine learning methods analyzed here fared little better. Including the mixed observations caused overall accuracy to markedly decline, going from `r summary_all %>% filter(source == "cs" & ppm == "rf" & scenario == "nomix_imbal") %>% pull(accuracy_pct) %>% round(digits = 1)`% (rain and snow only) to `r summary_all %>% filter(source == "cs" & ppm == "rf" & scenario == "allphase_imbal") %>% pull(accuracy_pct) %>% round(digits = 1)`% (rain, snow, and mixed) for random forest and from `r summary_all %>% filter(source == "cs" & ppm == "xg" & scenario == "nomix_imbal") %>% pull(accuracy_pct) %>% round(digits = 1)`% (rain and snow only) to `r summary_all %>% filter(source == "cs" & ppm == "xg" & scenario == "allphase_imbal") %>% pull(accuracy_pct) %>% round(digits = 1)`% (rain, snow, and mixed) for XGBoost. 

Although XGBoost had a higher accuracy than random forest when we included mixed precipitation, it achieved this result at the expense of bias. The random forest method predicted that `r summary_all %>% filter(source == "cs" & ppm == "rf" & scenario == "allphase_imbal") %>% pull(mixed_pred_pct) %>% round(digits = 1)`% of the observations were mixed, which was well short of\ the observed value of `r summary_all %>% filter(source == "cs" & ppm == "rf" & scenario == "allphase_imbal") %>% pull(mixed_obs_pct) %>% round(digits = 1)`%, giving it a mixed bias of `r summary_all %>% filter(source == "cs" & ppm == "rf" & scenario == "allphase_imbal") %>% pull(mixed_bias_pct) %>% round(digits = 1)`%. XGBoost fared even worse, predicting `r summary_all %>% filter(source == "cs" & ppm == "xg" & scenario == "allphase_imbal") %>% pull(mixed_pred_pct) %>% round(digits = 1)`% of the observations to be mixed, giving it a mixed bias of `r summary_all %>% filter(source == "cs" & ppm == "xg" & scenario == "allphase_imbal") %>% pull(mixed_bias_pct) %>% round(digits = 1)`%. When including mixed precipitation, random forest had a lower snow bias (`r summary_all %>% filter(source == "cs" & ppm == "rf" & scenario == "allphase_imbal") %>% pull(snow_bias_pct) %>% round(digits = 1)`%) than XGBoost (`r summary_all %>% filter(source == "cs" & ppm == "xg" & scenario == "allphase_imbal") %>% pull(snow_bias_pct) %>% round(digits = 1)`%). It was the same story for rain bias with XGBoost (`r summary_all %>% filter(source == "cs" & ppm == "xg" & scenario == "allphase_imbal") %>% pull(rain_bias_pct) %>% round(digits = 1)`%) having a worse bias than random forest (`r summary_all %>% filter(source == "cs" & ppm == "rf" & scenario == "allphase_imbal") %>% pull(rain_bias_pct) %>% round(digits = 1)`%). These findings appear even worse when viewing the confusion matrix in Figure 5. This plot shows that random forest correctly predicted only 9.3% of the observed mixed precipitation and incorrectly predicted the mixed observations as rain 32.9% of the time and as snow the remaining 57.8%. XGBoost failed completely here, correctly predicting none of the mixed precipitation observations, identifying them as rain and snow 36.8% and 63.2% of the time, respectively. 
...
```
This lack of differentiability reveals an issue with using near-surface meteorology to partition rain and snow. The left panels of Figure 8 below present the average accuracy of the benchmark and machine learning phase partitioning methods plus the overlap in rain and snow distributions from the two datasets. We plot both variables against air temperature to show how the dip in performance corresponds to the increase in overlap. What is more, the right panel indicates there is a strong negative relationship between the distribution overlap and average accuracy, where accuracy decreases as overlap increases. Using ordinary least squares regression, we compute an r^2^ of `r cs_overlap_accuracy_lm$r.squared %>% round(digits = 2)` for this relationship in the crowdsourced dataset and `r nh_overlap_accuracy_lm$r.squared %>% round(digits = 2)` in the synoptic. That means that the air temperature distribution overlap between rain and snow explains at least 73% of the variance in precipitation phase partitioning accuracy. The higher the overlap, the worse the performance.
