...
# Processing plot data for export

## Manuscript figure 1

``` r
fig01_data <- summary_bytemp %>% 
  filter(scenario == "nomix_imbal") %>% 
  filter(!ppm %in% c("xg", "rf", "nn")) %>% 
  left_join(ppm_labels, by = "ppm") %>% 
  mutate(source2 = ifelse(source == "cs", "Crowdsourced", "Synoptic"))
...
``` r
fig02_data <- ml_benchmark_diff_bytemp
```
...
``` r
fig05_data <- overlap_dens_tair_binned
```

## Other figures

Figures 6 (map) and 7 (workflow infographic) are not summarized here.
The data for the former are accessible via the source data links and the
latter has no underlying data.

## Supplementary figure 1

``` r
figSUP01_data <- summary_bytemp %>% 
  filter(scenario == "nomix_imbal") %>% 
  filter(ppm %in% c("xg", "rf", "nn")) %>% 
  left_join(ppm_labels, by = "ppm") %>% 
  mutate(source2 = ifelse(source == "cs", "Crowdsourced", "Synoptic"))
...
figSUP03_data <- 
  bind_rows(
    vip::vi(object =  ml_for_vip[[1]]) %>% 
      mutate(ppm = "xg",
             source = "Crowdsourced"),
    vip::vi(object =  ml_for_vip[[2]]) %>% 
      mutate(ppm = "rf",
             source = "Crowdsourced"),
    vip::vi(object =  ml_for_vip[[3]]) %>% 
      mutate(ppm = "xg",
             source = "Synoptic"),
    vip::vi(object =  ml_for_vip[[4]]) %>% 
      mutate(ppm = "rf",
             source = "Synoptic")
  )
```

## Combine into list and export
...
openxlsx::write.xlsx(x = fig_data_all, 
                     sheetName = sheetnames,
                     file = paste0(data_pre, "source_data.xlsx"))
```
