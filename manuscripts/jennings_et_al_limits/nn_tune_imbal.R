...
saveRDS(export_list, file = paste0(data_pre, "nn_tune_imbal_CS.RDS"))
...
# Save info from the tuning exercise
export_list <- list()
export_list[["nomix_imbal"]] <- list(nn_flow_tuned = nn_flow_tuned, 
                                     param_best = param_best,
                                     nn_tune_results = nn_tune_results %>% collect_metrics())

# Export the tuning results
saveRDS(export_list, file = paste0(data_pre, "nn_tune_imbal_NH.RDS"))
